...
export const getPrepSettings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    if (error) throw error;
    return (data ?? { absolute_min_pipette_ul: 10, preferred_min_pipette_ul: 20, default_calibration_levels: 6, default_target_level: 3, max_dilution_steps: 5, drive_lm_sample_prep_folder_id: null, drive_lm_reports_complete_folder_id: null, drive_hplc_results_folder_id: null }) as PrepSettings;
  });
...
export const updatePrepSettings = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({
    ...
    drive_hplc_results_folder_id: z.preprocess(
      extractDriveFolderId,
      z.string().max(200).regex(/^[A-Za-z0-9_-]*$/, "Invalid Drive folder ID").nullable().optional(),
    ),
  }).parse(d))
