...
const WEIGHTS_DAD_ONLY = {
  rt: 30, dadPurity: 35, mass: 0, isotopeAdduct: 0, msms: 0, stressContext: 10, evidencePrior: 5,
};
const WEIGHTS_LC_MS = {
  rt: 15, dadPurity: 15, mass: 35, isotopeAdduct: 10, msms: 10, stressContext: 3, evidencePrior: 2,
};
...
function scoreRt(peak: PeakInput, parentRt: number, candidate: NcCandidateInput, weight: number): number {
...
function scoreMass(
  ...
  if (weight === 0) return 0;
  if (peak.observedNeutralMass == null || parentMonoisotopicMass == null || candidate.massDelta == null) return 0;
...
function scoreStressContext(stressContext: string | null, likelyTrigger: string | null, weight: number): number {
  if (weight === 0 || !stressContext || !likelyTrigger) return 0;
  const stressWords = stressContext.toLowerCase().split(/[^a-z]+/).filter(w => w.length > 3);
...
export function rankCandidatesForPeak(args: {
  ...
  const { peak, parentPeakRt, parentMonoisotopicMass, candidates, stressContext } = args;
  ...
  const ranked = candidates.map((candidate): RankedCandidate => {
    ...
    const dadPurity = scoreDadPurity(peak, weights.dadPurity);
    const mass = scoreMass(peak, parentMonoisotopicMass, candidate, weights.mass);
    const stressContextScore = scoreStressContext(stressContext, candidate.likelyTrigger, weights.stressContext);
    const evidencePrior = scoreEvidencePrior(candidate.evidenceLevel, weights.evidencePrior);
    const maxPossible = weights.rt + weights.dadPurity + weights.mass + weights.isotopeAdduct
      + weights.msms + weights.stressContext + weights.evidencePrior;
    const total = rt + dadPurity + mass + stressContextScore + evidencePrior;
    const scores: ComponentScores = {
      rt, dadPurity, mass, isotopeAdduct: 0, msms: 0,
      stressContext: stressContextScore, evidencePrior, total, maxPossible,
    };
    return { candidate, scores, tier: tierFor(scores, hasObservedMass) };
  });
...
export function generateNextSteps(args: {
  ...
  const { topCandidates, spectralPanel, hasObservedMass } = args;
  ...
  if (topCandidates.length >= 2) {
    ...
    if (top.scores.total - second.scores.total < top.scores.maxPossible * 0.1) {
      suggestions.push({
        text: `Top candidates ("${top.candidate.name}" vs "${second.candidate.name}") are close in score. `
          + (spectralPanel && spectralPanel.wavelengthsNm.length
            ? `This compound's spectral panel recommends monitoring ${spectralPanel.wavelengthsNm.join("/")} nm`
              + (spectralPanel.recommendedFeatures ? ` and tracking ${spectralPanel.recommendedFeatures}` : "")
  ...
  if (oligomerHit) {
    suggestions.push({
      text: `"${oligomerHit.candidate.name}" is an oligomer/aggregation candidate — RP-HPLC/DAD alone cannot confirm this class. `
  ...
  if (!hasObservedMass && topCandidates.length > 0 && topCandidates[0].tier !== "candidate") {
    suggestions.push({
      text: "This finding scored high enough to be worth confirming — no mass data is on record for this peak. "
