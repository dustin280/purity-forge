...
import {
  rankCandidatesForPeak, generateNextSteps, LOW_AREA_PCT_THRESHOLD,
  type PeakInput, type NcCandidateInput, type RankedCandidate,
} from "./engine";
...
export const previewNcEvaluation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [{ data: compound, error: e1 }, { data: impurities, error: e2 }, { data: oligomers, error: e3 }, { data: panel, error: e4 }] = await Promise.all([
      supabase.from("nc_compounds").select("id, name, monoisotopic_mass").eq("id", data.nc_compound_id).single(),
      ...
      supabase.from("nc_spectral_panels").select("wavelengths_nm, recommended_features").eq("nc_compound_id", data.nc_compound_id).maybeSingle(),
    ]);
    if (e1) throw e1;
    if (e2) throw e2;
    if (e3) throw e3;
    ...
    const candidates: NcCandidateInput[] = [
      ...(impurities ?? []).map((r: NcRow): NcCandidateInput => ({
        id: r.id, kind: "impurity", name: r.name, evidenceLevel: r.evidence_level,
        rpHplcBehavior: r.rp_hplc_behavior, massDelta: r.mass_delta, likelyTrigger: r.likely_trigger,
      })),
      ...(oligomers ?? []).map((r: NcRow): NcCandidateInput => ({
        id: r.id, kind: "oligomer", name: r.name, evidenceLevel: r.evidence_level,
        rpHplcBehavior: r.rp_hplc_behavior, massDelta: r.mass_delta_vs_n_monomer, likelyTrigger: r.trigger_motif,
        falsePositiveWarning: r.false_positive_warning,
      })),
    ];

    const parentPeak = data.peaks.reduce((a, b) => (b.area_pct > a.area_pct ? b : a), data.peaks[0]);
    const extraPeaks = data.peaks.filter(p => p.peak_id !== parentPeak.peak_id && p.area_pct >= LOW_AREA_PCT_THRESHOLD);

    const findings: FindingPreview[] = extraPeaks.map(p => {
      const peak: PeakInput = {
        peakId: p.peak_id, rt: p.rt, areaPct: p.area_pct,
        peakPurity: p.peak_purity ?? null, peakPurityPassed: p.peak_purity_passed ?? null, uvMatch: p.uv_match ?? null,
      };
      const ranked = rankCandidatesForPeak({
        peak, parentPeakRt: parentPeak.rt, parentMonoisotopicMass: compound.monoisotopic_mass,
        candidates, stressContext: data.stress_context ?? null,
      });
      const nextSteps = generateNextSteps({
        topCandidates: ranked.slice(0, 3),
        spectralPanel: panel ? { wavelengthsNm: panel.wavelengths_nm ?? [], recommendedFeatures: panel.recommended_features } : null,
        hasObservedMass: false,
      });
      return {
        peak_id: p.peak_id, rt: p.rt, area_pct: p.area_pct,
        peak_purity: p.peak_purity ?? null, peak_purity_passed: p.peak_purity_passed ?? null, uv_match: p.uv_match ?? null,
        ranked, next_steps: nextSteps,
      };
    });
...
export const saveNcEvaluation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const supabase = context.supabase as AnySupabase;
    const { data: evalRow, error: e1 } = await supabase.from("nc_evaluations").insert({
      sample_id: data.sample_id, result_id: data.result_id, nc_compound_id: data.nc_compound_id,
      run_by: context.userId, run_by_name: data.run_by_name,
      stress_context: data.stress_context ?? null, summary: data.summary ?? null, overall_tier: data.overall_tier,
    }).select("id").single();
    if (e1) throw e1;
    ...
    if (data.findings.length) {
      const rows = data.findings.map(f => ({
        ...
        peak_id: f.peak_id, rt: f.rt, area_pct: f.area_pct,
        peak_purity: f.peak_purity, peak_purity_passed: f.peak_purity_passed, uv_match: f.uv_match,
        candidate_kind: f.candidate_kind, matched_candidate_id: f.matched_candidate_id,
        component_scores: f.component_scores, tier: f.tier,
        rationale: f.rationale ?? null, analyst_note: f.analyst_note ?? null,
      }));
...
export const listNcEvaluationsForSample = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    return (rows ?? []) as Array<{
      id: string; run_by_name: string; run_at: string; overall_tier: string; summary: string | null;
  ...
  .handler(async ({ context, data }) => {
    ...
    const [{ data: sample }, { data: result }] = await Promise.all([
      evaluation.sample_id
        ? supabase.from("samples").select("batch_id, client").eq("id", evaluation.sample_id).maybeSingle()
        : Promise.resolve({ data: null }),
      evaluation.result_id
        ? supabase.from("results").select("chromatogram_image").eq("id", evaluation.result_id).maybeSingle()
        : Promise.resolve({ data: null }),
    ]);

    const impurityIds = (findings ?? []).filter((f: NcRow) => f.candidate_kind === "impurity" && f.matched_candidate_id).map((f: NcRow) => f.matched_candidate_id);
    const oligomerIds = (findings ?? []).filter((f: NcRow) => f.candidate_kind === "oligomer" && f.matched_candidate_id).map((f: NcRow) => f.matched_candidate_id);
    const [{ data: impurities }, { data: oligomers }] = await Promise.all([
      impurityIds.length ? supabase.from("nc_impurity_candidates").select("*").in("id", impurityIds) : Promise.resolve({ data: [] }),
      oligomerIds.length ? supabase.from("nc_oligomer_candidates").select("*").in("id", oligomerIds) : Promise.resolve({ data: [] }),
    ]);
    const candidateById = new Map<string, NcRow>([
      ...(impurities ?? []).map((c: NcRow) => [c.id, { ...c, kind: "impurity" }] as [string, NcRow]),
    ...
    return {
      ...
      findings: (findings ?? []).map((f: NcRow) => ({
        ...f,
        candidate: f.matched_candidate_id ? candidateById.get(f.matched_candidate_id) ?? null : null,
      })),
      sample: sample ?? null,
