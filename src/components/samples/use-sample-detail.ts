...
export function useSampleDetail(batchId: string) {
  ...
  async function submitResult(args: {
    ...
    const { peaks, purity, raw_data_file_path, analysis_date, chromatogram_image, calibration_image, calibration_data, calibration_curves, uv_conf_match, wavelength_nm, report_metadata } = args.imported
      ? {
          ...
          calibration_curves: args.imported.calibration_curves,
      ...
      : {
          ...
          calibration_curves: null as CalibrationCurve[] | null,
    ...
    setBusy(true);
    try {
      await saveResultFn({ data: { testId: args.testId, purity_percentage: purity, peaks, raw_data_file_path, analysis_date, chromatogram_image, calibration_image, calibration_data, calibration_curves, uv_conf_match, wavelength_nm, report_metadata } });
