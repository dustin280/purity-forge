...
import type { AqueousFlowState } from "./types";
...
interface Props {
  state: AqueousFlowState;
  dilutionResult: DilutionResult;
  preparedAt: string;
  analystName: string;
}

/** Printable review card. Mirrors working-flow/step-review.tsx, swapping the
 * "diluted from a primary" source section for one read from a material
 * receipt instead. */
    ...
    <div ref={ref} className="space-y-4 print:space-y-3">
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-start justify-between border-b pb-3">
          <div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground">Primary Standard Preparation (Aqueous)</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Section title="Source (Material Receipt)">
            <Row label="Material" value={src.material_name} />
            <Row label="Receipt" value={src.receipt_number} />
            <Row label="Lot" value={src.internal_lot || src.manufacturer_lot || "—"} />
            <Row label="Manufacturer" value={src.manufacturer || "—"} />
            <Row label="Stock concentration" value={src.stock_concentration_mg_per_ml != null ? `${src.stock_concentration_mg_per_ml} mg/mL` : "—"} />
          </Section>
