...
import {
  ...
  recordJournalAttachment,
  signJournalAttachment,
...
export function AttachmentPanel({ entryId, userId }: AttachmentPanelProps) {
  const qc = useQueryClient();
  const fileInput = useRef<HTMLInputElement | null>(null);
  const [uploading, setUploading] = useState(false);
  const list = useServerFn(listJournalAttachments);
  const record = useServerFn(recordJournalAttachment);
  const sign = useServerFn(signJournalAttachment);
  const del = useServerFn(deleteJournalAttachment);

  const query = useQuery({
    queryKey: entryId
      ? qk.labJournalAttachments.list(entryId)
      : ["lab-journal-attachments", "none"],
    queryFn: () => list({ data: { entry_id: entryId as string } }),
    enabled: !!entryId,
  });

  const rows = useMemo<LabJournalAttachment[]>(
    () => query.data ?? [],
    [query.data],
  );
    ...
    onSuccess: () => {
      ...
      if (entryId)
        qc.invalidateQueries({
          queryKey: qk.labJournalAttachments.list(entryId),
        });
    },
