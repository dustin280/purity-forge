-- Hourly automated report reconciliation: adds a shared secret for the
-- cron endpoint to authenticate with (no user session exists when
-- pg_cron/pg_net call it), then schedules the hourly job itself.
